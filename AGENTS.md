...
| `src/reaper_mcp/installer.py` | Copies `reaper_bridge.lua` + `reaper_project/default.RPP` into the REAPER install, wires both into `__startup.lua` |
| `reaper_project/default.RPP` | Blank, track-less project checked into the repo, generated by REAPER itself (not hand-authored); opened automatically on every REAPER launch |
...
- **`track_index: -1` means the master track**, everywhere a track-taking op
  ...
  inspecting/adjusting master volume the same way as any track).
